...
const getNotes = async () => {
  ...
  if (lodashExports.isEmpty(notes)) {
    ...
    await fsExtra.writeFile(path.join(rootDir, welcomeNoteFilename), content, { encoding: fileEncoding });
    notes.push(welcomeNoteFilename);
  }
  return Promise.all(notes.map(getNoteInfoFromFilename));
};
const getNoteInfoFromFilename = async (filename) => {
  const fileStats = await fsExtra.stat(path.join(getRootDir(), filename));
  return {
...
const readNote = async (filename) => {
  const rootDir = getRootDir();
  return fsExtra.readFile(path.join(rootDir, `${filename}.md`), { encoding: fileEncoding });
};
const writeNote = async (filename, content) => {
  const rootDir = getRootDir();
  console.info(`Writing note ${filename}`);
  return fsExtra.writeFile(path.join(rootDir, `${filename}.md`), content, { encoding: fileEncoding });
};
...
const deleteNote = async (filename) => {
  ...
  await fsExtra.remove(path.join(rootDir, `${filename}.md`));
  return true;
};
...
function createWindow() {
  const mainWindow = new electron.BrowserWindow({
    ...
    title: "aegis",
    frame: false,
    // Ensure the frame is false if you are using a custom title bar
